...
public class MotionController {
    ...
    public void setup(int parentWidth, int parentHeight, float transitionDuration, long currentTime) {
        ...
        while (true) {
            ...
            int i3 = 0;
            while (true) {
                if (i3 >= motionPathsArr.length) {
                    break;
                }
                if (motionPathsArr[i3].attributes.containsKey(str6) && (constraintAttribute = motionPathsArr[i3].attributes.get(str6)) != null) {
                    int[] iArr = this.mAttributeInterpolatorCount;
                    iArr[i2] = iArr[i2] + constraintAttribute.numberOfInterpolatedValues();
                    break;
                }
                i3++;
            }
        ...
        for (int i4 = 1; i4 < motionPathsArr.length; i4++) {
            motionPathsArr[i4].different(motionPathsArr[i4 - 1], zArr, this.mAttributeNames, z);
        }
        int i5 = 0;
        ...
        this.mInterpolateVariables = new int[i5];
        ...
        for (int i8 = 1; i8 < zArr.length; i8++) {
            if (zArr[i8]) {
                this.mInterpolateVariables[i7] = i8;
                i7++;
            }
        }
        ...
        for (int i9 = 0; i9 < motionPathsArr.length; i9++) {
            motionPathsArr[i9].fillStandard(dArr[i9], this.mInterpolateVariables);
            dArr2[i9] = motionPathsArr[i9].time;
        }
        int i10 = 0;
        while (true) {
            ...
            if (iArr2[i10] < MotionPaths.names.length) {
                String str7 = MotionPaths.names[this.mInterpolateVariables[i10]] + " [";
                for (int i11 = 0; i11 < motionPathsArr.length; i11++) {
                    str7 = str7 + dArr[i11][i10];
                }
            }
            i10++;
        }
            ...
            for (int i14 = 0; i14 < motionPathsArr.length; i14++) {
                if (motionPathsArr[i14].hasCustomData(str8)) {
                    if (dArr4 == null) {
                        ...
                        dArr4 = (double[][]) Array.newInstance(double.class, motionPathsArr.length, motionPathsArr[i14].getCustomDataCount(str8));
                    }
                    dArr3[i13] = motionPathsArr[i14].time;
                    motionPathsArr[i14].getCustomData(str8, dArr4[i13], 0);
                    i13++;
                }
            }
            i12++;
            this.mSpline[i12] = CurveFit.get(this.mCurveFitType, Arrays.copyOf(dArr3, i13), (double[][]) Arrays.copyOf(dArr4, i13));
        }
        ...
        if (motionPathsArr[0].mPathMotionArc != Key.UNSET) {
            ...
            for (int i15 = 0; i15 < length; i15++) {
                iArr3[i15] = motionPathsArr[i15].mPathMotionArc;
                dArr5[i15] = motionPathsArr[i15].time;
                dArr6[i15][0] = motionPathsArr[i15].x;
                dArr6[i15][1] = motionPathsArr[i15].y;
            }
    ...
    public void endTrigger(boolean start) {
        if (!ThemeManager.AttributeSet.BUTTON.equals(Debug.getName(this.mView)) || this.mKeyTriggers == null) {
            return;
        }
        int i = 0;
        while (true) {
