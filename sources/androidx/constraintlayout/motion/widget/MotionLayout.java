...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    class DecelerateInterpolator extends MotionInterpolator {
        ...
        public float getInterpolation(float time) {
            ...
            if (f3 > 0.0f) {
                ...
                float f6 = this.maxA;
                motionLayout.mLastVelocity = f5 - (f6 * time);
                f = (f5 * time) - (((f6 * time) * time) / 2.0f);
                f2 = this.currentP;
            } else {
                float f7 = this.maxA;
                if ((-f3) / f7 < time) {
                ...
                float f9 = this.maxA;
                motionLayout2.mLastVelocity = (f9 * time) + f8;
                f = (f8 * time) + (((f9 * time) * time) / 2.0f);
                f2 = this.currentP;
            }
            return f + f2;
        }
        ...
        if (this.mBeginState == id) {
            ...
            if (staggered != 0.0f) {
                ...
                for (int i7 = 0; i7 < childCount; i7++) {
                    ...
                    motionController5.mStaggerOffset = staggered - ((((finalX + finalY2) - f2) * staggered) / (f - f2));
                }
            }
            this.mTransitionPosition = 0.0f;
            this.mTransitionLastPosition = 0.0f;
    ...
    public void getViewVelocity(View view, float posOnViewX, float posOnViewY, float[] returnVelocity, int type) {
        ...
        if (this.mInterpolator != null) {
            ...
            f = (signum * ((interpolation - interpolation2) / EPSILON)) / this.mTransitionDuration;
            f2 = interpolation2;
        }
        Interpolator interpolator = this.mInterpolator;
        if (interpolator instanceof MotionInterpolator) {
            f = ((MotionInterpolator) interpolator).getVelocity();
        }
        float f3 = f;
    ...
    public class Model {
        ...
        public void measure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (((MotionLayout.this.getParent() instanceof MotionLayout) && mode == 1073741824 && mode2 == 1073741824) ? false : true) {
                ...
                MotionLayout motionLayout2 = MotionLayout.this;
                motionLayout2.mMeasureDuringTransition = (motionLayout2.mStartWrapWidth == MotionLayout.this.mEndWrapWidth && MotionLayout.this.mStartWrapHeight == MotionLayout.this.mEndWrapHeight) ? false : true;
            }
