...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        this.mMode = 1;
        this.x = (int) ((startTimePoint.x + f14) - f15);
        this.y = (int) ((startTimePoint.y + f16) - f17);
        this.x += (-f13) * f18;
        this.y += f12 * f18;
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, int offset) {
        ...
        if (motionController != null) {
            ...
            double d = f8;
            double d2 = f7;
            f = (float) ((f10 + (Math.sin(d2) * d)) - (f6 / 2.0f));
            f7 = (float) ((f11 - (d * Math.cos(d2))) - (f5 / 2.0f));
        } else {
            f = f8;
        }
        point[offset] = f + (f6 / 2.0f) + 0.0f;
        point[offset + 1] = f7 + (f5 / 2.0f) + 0.0f;
    }
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, double[] vdata, float[] velocity) {
        ...
        float f11 = 0.0f;
        for (int i = 0; i < toUse.length; i++) {
            float f12 = (float) data[i];
            float f13 = (float) vdata[i];
            int i2 = toUse[i];
            if (i2 == 1) {
                f4 = f12;
                f8 = f13;
        ...
        if (motionController != null) {
            ...
            double d = f4;
            double d2 = f5;
            float sin = (float) ((f17 + (Math.sin(d2) * d)) - (f6 / 2.0f));
            float cos = (float) ((f18 - (d * Math.cos(d2))) - (f7 / 2.0f));
            double d3 = f8;
            f2 = sin;
            double d4 = f10;
            float sin2 = (float) (f19 + (Math.sin(d2) * d3) + (Math.cos(d2) * d4));
            f = (float) ((f20 - (d3 * Math.cos(d2))) + (Math.sin(d2) * d4));
            f5 = cos;
            f15 = sin2;
            f14 = 2.0f;
        } else {
        ...
        point[0] = f2 + (f6 / f14) + 0.0f;
        point[1] = f5 + (f7 / f14) + 0.0f;
        velocity[0] = f15;
        velocity[1] = f;
    }
    ...
    public void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        while (true) {
            ...
            if (Double.isNaN(dArr[i3]) && (cycle == null || cycle[i3] == 0.0d)) {
                ...
                f5 = f10;
                float f19 = (float) d;
                float f20 = (float) this.mTempDelta[i3];
                if (i3 != 0) {
                    if (i3 == 1) {
                        f10 = f5;
                        f18 = f19;
        ...
        if (motionController != null) {
            ...
            double d2 = f18;
            double d3 = f17;
            f3 = (float) ((f22 + (Math.sin(d3) * d2)) - (f11 / 2.0f));
            double d4 = f24;
            f4 = (float) ((f23 - (Math.cos(d3) * d2)) - (f12 / 2.0f));
            double d5 = f13;
            f2 = f12;
            f = f11;
            double d6 = f14;
            float sin = (float) (d4 + (Math.sin(d3) * d5) + (Math.cos(d3) * d2 * d6));
            float cos = (float) ((f25 - (d5 * Math.cos(d3))) + (d2 * Math.sin(d3) * d6));
            if (slope.length >= 2) {
                slope[0] = sin;
                z = true;
                slope[1] = cos;
            ...
            if (!Float.isNaN(f21)) {
                view.setRotation((float) (f21 + Math.toDegrees(Math.atan2(cos, sin))));
            }
        } else {
            f = f11;
            f2 = f12;
            z = true;
            if (!Float.isNaN(f21)) {
                view.setRotation((float) (0.0f + f21 + Math.toDegrees(Math.atan2(f14 + (f16 / 2.0f), f13 + (f15 / 2.0f)))));
            }
            f3 = f18;
            f4 = f17;
        }
        if (view instanceof FloatLayout) {
            ((FloatLayout) view).layout(f3, f4, f3 + f, f4 + f2);
            return;
        }
        float f26 = f3 + 0.5f;
        int i4 = (int) f26;
