    ...
    private void fill(TypedArray a) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (index == R.styleable.OnSwipe_touchAnchorId) {
                ...
                this.mTouchAnchorSide = a.getInt(index, this.mTouchAnchorSide);
                float[][] fArr = TOUCH_SIDES;
                int i2 = this.mTouchAnchorSide;
                this.mTouchAnchorX = fArr[i2][0];
                this.mTouchAnchorY = fArr[i2][1];
            } else if (index == R.styleable.OnSwipe_dragDirection) {
                this.mTouchSide = a.getInt(index, this.mTouchSide);
    ...
    */
    ...
    public void processTouchEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        if (action == 0) {
            ...
            this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, f8, f);
            ...
            if (Math.abs((this.mTouchDirectionX * rawX) + (this.mTouchDirectionY * rawY)) > this.mDragThreshold || this.mDragStarted) {
                ...
                if (max != progress3) {
                    int i3 = (progress3 > 0.0f ? 1 : (progress3 == 0.0f ? 0 : -1));
                    if (i3 == 0 || progress3 == 1.0f) {
                        this.mMotionLayout.endTrigger(i3 == 0);
                    }
    ...
    public void scrollUp(float dx, float dy) {
        ...
        if (progress != 0.0f) {
            if ((this.mOnTouchUp != 3) && (progress != 1.0f)) {
                this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, ((double) progress) < 0.5d ? 0.0f : 1.0f, f);
            }
        }
    }
