    ...
    static final class Default extends Tracing {
        ...
        Default(Builder builder) {
            ...
            if (builder.spanReporter != Reporter.NOOP) {
                finishedSpanHandler = new ZipkinFinishedSpanHandler(builder.spanReporter, this.errorParser, builder.localServiceName, builder.localIp, builder.localPort);
                ArrayList arrayList = new ArrayList(list);
                arrayList.add(finishedSpanHandler);
                list = arrayList;
            }
            FinishedSpanHandler noopAware = FinishedSpanHandlers.noopAware(FinishedSpanHandlers.compose(list), this.noop);
            this.tracer = new Tracer(builder.clock, builder.propagationFactory, noopAware, new PendingSpans(this.clock, finishedSpanHandler, this.noop), builder.sampler, builder.currentTraceContext, builder.traceId128Bit || this.propagationFactory.requires128BitTraceId(), builder.supportsJoin && this.propagationFactory.supportsJoin(), noopAware.alwaysSampleLocal(), this.noop);
            maybeSetCurrent();
        }
