...
public class StartPerfUtils {
    ...
    public static void appOnCreateEnd() {
        appCreateEndStamp = System.currentTimeMillis();
        appCreateTime = appCreateEndStamp - appCreateBeginStamp;
    }

    public static void onCreateBegin() {
        mainActCreateBeginStamp = System.currentTimeMillis();
        appMainActTime = mainActCreateBeginStamp - appCreateEndStamp;
    }

    public static void onCreateEnd() {
        mainActCreateEndStamp = System.currentTimeMillis();
        mainActCreateTime = mainActCreateEndStamp - mainActCreateBeginStamp;
    }

    public static void firstFrameEnd(Context context) {
        firstFrameEndStamp = System.currentTimeMillis();
        long j = firstFrameEndStamp;
        mainActFirstFrameTime = j - mainActCreateBeginStamp;
        appFirstFrameTime = j - appCreateBeginStamp;
        if (mIsColdStart) {
            mIsColdStart = false;
        } else {
            appCreateTime = -1L;
    ...
    public static void onStartEnd() {
        mainActStartEndStamp = System.currentTimeMillis();
    ...
    public static void onReStartEnd() {
        mainActRestartEndStamp = System.currentTimeMillis();
        mainActRestartTime = mainActRestartEndStamp - mainActRestartBeginStamp;
    }
    ...
    public static void onResumeEnd() {
        mainActResumeEndStamp = System.currentTimeMillis();
