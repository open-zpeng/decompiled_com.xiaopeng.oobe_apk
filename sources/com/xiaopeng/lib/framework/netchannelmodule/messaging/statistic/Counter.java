...
public class Counter {
    ...
    public Counter(Context context, BackgroundHandler backgroundHandler, boolean z) {
        ...
        try {
            this.mTryToPublishCount.set(defaultSharedPreferences.getLong(KEY_TRY_PUBLISH, 0L));
            this.mPublishFailedCount.set(defaultSharedPreferences.getLong(KEY_PUBLISH_FAILED, 0L));
            this.mPublishedCount.set(defaultSharedPreferences.getLong(KEY_PUBLISHED, 0L));
            this.mReceivedCount.set(defaultSharedPreferences.getLong(KEY_RECEIVED, 0L));
            this.mInitFailedCount.set(defaultSharedPreferences.getLong(KEY_INIT_FAILED, 0L));
            this.mPublishedSize.set(defaultSharedPreferences.getLong(KEY_PUBLISHED_SIZE, 0L));
    ...
    public void tryToCommitData(boolean z) {
        ...
        if (z || j3 >= MAX_VALUE_THRESHOLD || this.mLastDate.get() < currentTimeMillis) {
            ...
            iDataLog.sendStatData(iDataLog.buildMoleEvent().setEvent(EVENT_NAME).setPageId(GlobalConfig.MOLE_PAGE_ID).setButtonId("B003").setProperty(STAT_KEY_PACKAGE_NAME, GlobalConfig.getApplicationSimpleName()).setProperty(STAT_KEY_TRY_PUBLISH, Long.valueOf(j)).setProperty(STAT_KEY_PUBLISH_FAILED, Long.valueOf(j5)).setProperty(STAT_KEY_PUBLISHED, Long.valueOf(j4)).setProperty(STAT_KEY_PUBLISHED_SIZE_KB, Long.valueOf(j3 >> MIN_COMMIT_COUNT)).setProperty(STAT_KEY_RECEIVED, Long.valueOf(j2)).setProperty(STAT_KEY_INIT_FAILED, Long.valueOf(j6)).setProperty("date", Long.valueOf(j7)).setProperty(STAT_KEY_NETWORK_TYPE, Integer.valueOf(GlobalConfig.getNetworkType())).build());
        }
    }
