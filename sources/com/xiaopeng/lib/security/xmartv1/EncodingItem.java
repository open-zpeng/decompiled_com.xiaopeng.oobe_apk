...
public final class EncodingItem {
    ...
    public static EncodingItem decode(byte[] bArr) {
        ...
        if (wrap.get() != sMagicNumber[0] || wrap.get() != sMagicNumber[1]) {
            ...
            encodingItem.mAesIv = new byte[wrap.get()];
            wrap.get(encodingItem.mAesIv);
            ...
            if (decryptWithByteBuffer != null && decryptWithByteBuffer.length > 0) {
                ...
                encodingItem.mRawContent = new byte[decryptWithByteBuffer.length - encodingItem.mNonce.length];
                wrap2.get(encodingItem.mRawContent);
                return encodingItem;
            }
            return sEmptyItem;
        }
