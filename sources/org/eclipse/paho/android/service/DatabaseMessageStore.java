...
public class DatabaseMessageStore implements MessageStore {
    ...
    public String storeArrived(String clientHandle, String topic, MqttMessage message) {
        ...
        MqttTraceHandler mqttTraceHandler = this.traceHandler;
        mqttTraceHandler.traceDebug(TAG, "storeArrived{" + clientHandle + "}, {" + message.toString() + "}");
    ...
    public boolean discardArrived(String clientHandle, String id) {
        this.db = this.mqttDb.getWritableDatabase();
        MqttTraceHandler mqttTraceHandler = this.traceHandler;
        mqttTraceHandler.traceDebug(TAG, "discardArrived{" + clientHandle + "}, {" + id + "}");
        try {
            int delete = this.db.delete(ARRIVED_MESSAGE_TABLE_NAME, "messageId=? AND clientHandle=?", new String[]{id, clientHandle});
            if (delete != 1) {
                MqttTraceHandler mqttTraceHandler2 = this.traceHandler;
                mqttTraceHandler2.traceError(TAG, "discardArrived - Error deleting message {" + id + "} from database: Rows affected = " + delete);
                return false;
            }
            int arrivedRowCount = getArrivedRowCount(clientHandle);
    ...
    public void clearArrivedMessages(String clientHandle) {
        ...
        if (clientHandle == null) {
            ...
            MqttTraceHandler mqttTraceHandler = this.traceHandler;
            mqttTraceHandler.traceDebug(TAG, "clearArrivedMessages: clearing the table of " + clientHandle + " messages");
            delete = this.db.delete(ARRIVED_MESSAGE_TABLE_NAME, "clientHandle=?", strArr);
        }
        MqttTraceHandler mqttTraceHandler2 = this.traceHandler;
        mqttTraceHandler2.traceDebug(TAG, "clearArrivedMessages: rows affected = " + delete);
    }
