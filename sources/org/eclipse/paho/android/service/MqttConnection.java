...
public class MqttConnection implements MqttCallbackExtended {
        ...
        try {
            ...
            MqttConnectionListener mqttConnectionListener = new MqttConnectionListener(bundle) { // from class: org.eclipse.paho.android.service.MqttConnection.1
                ...
                public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
                    ...
                    mqttService2.traceError(MqttConnection.TAG, "connect fail, call connect to reconnect.reason:" + exception.getMessage());
    ...
    public IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained, String invocationContext, String activityToken) {
        ...
        if (mqttAsyncClient != null && mqttAsyncClient.isConnected()) {
            ...
            try {
                ...
                publish = this.myClient.publish(topic, payload, qos, retained, invocationContext, mqttConnectionListener);
            } catch (Exception e) {
                e = e;
            }
            try {
    ...
    public IMqttDeliveryToken publish(String topic, MqttMessage message, String invocationContext, String activityToken) {
        ...
        if (mqttAsyncClient != null && mqttAsyncClient.isConnected()) {
            try {
                publish = this.myClient.publish(topic, message, invocationContext, new MqttConnectionListener(bundle));
            } catch (Exception e) {
                e = e;
            }
            try {
    ...
    public void subscribe(final String topic, final int qos, String invocationContext, String activityToken) {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "subscribe({" + topic + "}," + qos + ",{" + invocationContext + "}, {" + activityToken + "}");
        Bundle bundle = new Bundle();
    ...
    public void subscribe(final String[] topic, final int[] qos, String invocationContext, String activityToken) {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "subscribe({" + Arrays.toString(topic) + "}," + Arrays.toString(qos) + ",{" + invocationContext + "}, {" + activityToken + "}");
        Bundle bundle = new Bundle();
        bundle.putString(MqttServiceConstants.CALLBACK_ACTION, MqttServiceConstants.SUBSCRIBE_ACTION);
        bundle.putString(MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken);
        bundle.putString(MqttServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext);
    ...
    public void subscribe(String[] topicFilters, int[] qos, String invocationContext, String activityToken, IMqttMessageListener[] messageListeners) {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "subscribe({" + Arrays.toString(topicFilters) + "}," + Arrays.toString(qos) + ",{" + invocationContext + "}, {" + activityToken + "}");
        Bundle bundle = new Bundle();
        bundle.putString(MqttServiceConstants.CALLBACK_ACTION, MqttServiceConstants.SUBSCRIBE_ACTION);
        bundle.putString(MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken);
        bundle.putString(MqttServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext);
    ...
    public void unsubscribe(final String topic, String invocationContext, String activityToken) {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "unsubscribe({" + topic + "},{" + invocationContext + "}, {" + activityToken + "})");
        Bundle bundle = new Bundle();
    ...
    public void unsubscribe(final String[] topic, String invocationContext, String activityToken) {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "unsubscribe({" + Arrays.toString(topic) + "},{" + invocationContext + "}, {" + activityToken + "})");
        Bundle bundle = new Bundle();
        bundle.putString(MqttServiceConstants.CALLBACK_ACTION, MqttServiceConstants.UNSUBSCRIBE_ACTION);
        bundle.putString(MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken);
        bundle.putString(MqttServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext);
        ...
        try {
            if (!this.connectOptions.isAutomaticReconnect()) {
                this.myClient.disconnect(null, new IMqttActionListener() { // from class: org.eclipse.paho.android.service.MqttConnection.2
                    ...
                    public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
                        MqttService mqttService2 = MqttConnection.this.service;
                        mqttService2.traceDebug(MqttConnection.TAG, "disconnect onFailure, exception:" + exception);
                    }
                });
            } else {
    ...
    public void deliveryComplete(IMqttDeliveryToken messageToken) {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "deliveryComplete(" + messageToken + ")");
    ...
    public void messageArrived(String topic, MqttMessage message) throws Exception {
        MqttService mqttService = this.service;
        mqttService.traceDebug(TAG, "messageArrived(" + topic + ",{" + message.toString() + "})");
