...
public class ClipDrawable extends DrawableWrapper {
    ...
    private void initPath(int i, Rect rect) {
        ...
        if (this.mHotspotBounds.width() != 0 && this.mHotspotBounds.width() < width) {
            width = this.mHotspotBounds.width();
        }
        float f = width - (((10000 - i) * width) / 10000.0f);
        int height = rect.height();
        this.mClipPath = generateRoundRect(rect.left, rect.top, Math.max(this.mHotspotBounds.left, rect.left) + f, rect.top + height, this.mRadiusX, this.mRadiusY);
    }
