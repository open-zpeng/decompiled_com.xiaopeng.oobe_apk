...
public class SimpleSlider extends View {
    ...
    private void init(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        this.dragMax = obtainStyledAttributes.getFloat(R.styleable.SimpleSlider_drag_max, this.mMax);
    ...
    public void setTickMartPositions(@NonNull String str) {
        String[] split = str.split("\\D");
        Log.d(TAG, "setTickMartPositions: " + Arrays.toString(split) + ", string:" + str);
        if (split.length != 0) {
            this.mTickMarkPositions = new HashSet(split.length);
            this.mTickMarkPositions.addAll(Arrays.asList(split));
        }
        invalidate();
    }
    ...
    public void setTouchEventHandler(TouchEventHandler touchEventHandler) {
        ...
        if (touchEventHandler2 instanceof SimpleTouchEventHandler) {
            ((SimpleTouchEventHandler) touchEventHandler2).setSimpleSlider(this);
        }
    }
    ...
    public void setProgressFromTouch(float f, int i, boolean z, boolean z2, boolean z3) {
        ...
        if (i2 != 1) {
            if (i2 == 2) {
                ...
                if (z2) {
                    setProgressInternal(limitProgress, z, z3);
                    return;
                }
                setVisualProgress(16908301, getVisualScaleFromTouch(f, i));
                if (Math.abs(min - this.mProgress) < 1.0f || limitProgress == this.mProgress) {
                    return;
                }
            ...
            if (limitProgress2 != getProgress()) {
                setProgressInternal(limitProgress2, z, false);
                return;
            }
            return;
        }
        ...
        if (z2 || (Math.abs(min - getProgress()) >= 1.0f && limitProgress3 != this.mProgress)) {
            Log.i(TAG, "setProgressFromTouch, SET floatProgress:" + min + ", current:" + getProgress() + ", newProgress:" + limitProgress3);
            setProgressAndDispatch(limitProgress3, z);
        }
    }
    ...
    protected void setVisualProgress(int i, @FloatRange(from = 0.0d, to = 1.0d) float f) {
        Drawable drawable;
    ...
    private void animateProgress(final int i, final boolean z) {
        ...
        this.mValueAnimator = ValueAnimator.ofFloat(currentVisualScale, visualScale);
    ...
    protected boolean canDrawTickMark(int i) {
        ...
        if (i2 == 0) {
            if (!this.mTickMarkProgressEnd || i <= this.mProgress) {
                return this.mTickMarkProgressEnd || i < this.mProgress;
            }
            return false;
        } else if (i2 == 2) {
            Set<String> set = this.mTickMarkPositions;
